  ...
  const TaskComponent = ({ task }: { task: Task }) => (
      ...
      >
        <button
          ...
          onClick={async () => { await deleteTask(tasks, setTasks, task._id); }}
          type="button"
        >
          {deleteIcon}
        </button>
        <input
          ...
          onChange={async () => { await editTask.status(task, tasks, setTasks); }}
          type="checkbox"
        />
    ...
    <div className="container px-4 mx-auto">
      <header className="flex flex-row gap-4">
        ...
        <button
          ...
          onClick={async () => { await addTask(tasks, setTasks); }}
          type="button"
        >
          {addIcon}
          Add task
        ...
        {tasks.length > 0 && (
          <button
            ...
            onClick={async () => { await deleteAll(tasks, setTasks); }}
            type="button"
          >
            {deleteAllIcon}
            Remove all
